...
export const githubRepoSourceRegex =
...
export const redactApiKeys = (src: string) => {
  ...
  return src
    .replace(
      /licenseKey\s+["']?cfxk_\w{1,60}_(\w+)["']?.?$/gim,
      "licenseKey [REDACTED cfxk...$1]"
    )
    .replace(
      /steam_webApiKey\s+["']?\w{32}["']?.?$/gim,
      "steam_webApiKey [REDACTED]"
    )
    .replace(
      /sv_tebexSecret\s+["']?\w{40}["']?.?$/gim,
      "sv_tebexSecret [REDACTED]"
    )
    .replace(
      /rcon_password\s+["']?[^"']+["']?.?$/gim,
    ...
    .replace(
      /mysql_connection_string\s+["']?[^"']+["']?.?$/gim,
      "mysql_connection_string [REDACTED]"
    )
    .replace(
      /discord\.com\/api\/webhooks\/\d{17,20}\/\w{10,}.?$/gim,
 ...
 */
export const anyUndefined = (...args: any) =>
  [...args].some((x) => typeof x === "undefined");
...
export const calcExpirationFromDuration = (inputDuration: string) => {
  ...
  if (inputDuration === "permanent") {
    ...
    if (unit.startsWith("hour")) {
      ...
      throw new Error(
        `Invalid ban duration. Supported units: hours, days, weeks, months`
      );
    }
...
export const parsePlayerId = (idString: string) => {
  ...
  const validator =
    consts.validIdentifiers[idType as keyof typeof consts.validIdentifiers];
  ...
  for (const input of inputs) {
    if (input.includes(":")) {
      if (consts.regexValidHwidToken.test(input)) {
        ...
        Object.values(consts.validIdentifiers).some((regex) =>
          regex.test(input)
        )
      ) {
        validIds.push(input);
