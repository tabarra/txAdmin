...
const toDefault = (input, defVal) => {
  return isUndefined(input) ? defVal : input;
};
  ...
  if (await fse.pathExists(deployPath)) {
    ...
    if (pathFiles.some((x) => x !== ".empty")) {
      ...
      if (await canCreateFile(deployPath)) {
        ...
        throw new Error(
          "Path exists, but its not a folder, or its not writtable."
        );
      }
    }
  } else {
    if (await canCreateFile(deployPath)) {
      ...
      throw new Error(
        "Path doesn't exist, and we could not create it. Please check parent folder permissions."
      );
    }
  }
};
...
export const parseValidateRecipe = (rawRecipe) => {
  ...
  if (typeof recipe !== "object")
    throw new Error("invalid YAML, couldn't resolve to object");
  ...
  if (typeof recipe["$onesync"] == "string") {
    ...
    if (!["off", "legacy", "on"].includes(onesync))
      throw new Error(
        `the onesync option selected required for this recipe ("${onesync}") is not supported by this FXServer version.`
      );
    outRecipe.onesync = onesync;
  }
  if (typeof recipe["$minFxVersion"] == "number") {
    if (recipe["$minFxVersion"] > txEnv.fxServerVersion)
      throw new Error(
        `this recipe requires FXServer v${recipe["$minFxVersion"]} or above`
      );
    outRecipe.fxserverMinVersion = recipe["$minFxVersion"]; //NOTE: currently no downstream use
  }
  if (typeof recipe["$engine"] == "number") {
    if (recipe["$engine"] < engineVersion)
  ...
  recipe.tasks.forEach((task, index) => {
    if (typeof task.action !== "string")
      throw new Error(`[task${index + 1}] no action specified`);
    if (typeof recipeEngine[task.action] === "undefined")
      throw new Error(`[task${index + 1}] unknown action '${task.action}'`);
    if (!recipeEngine[task.action].validate(task))
      throw new Error(`[task${index + 1}:${task.action}] invalid parameters`);
    outRecipe.tasks.push(task);
  });

  //Process inputs
  outRecipe.requireDBConfig = recipe.tasks.some((t) =>
    t.action.includes("database")
  );
  const protectedVarNames = [
    "licenseKey",
    "dbHost",
    "dbUsername",
    "dbPassword",
  ...
  if (typeof recipe.variables == "object" && recipe.variables !== null) {
    ...
    if (varNames.some((n) => protectedVarNames.includes(n))) {
      throw new Error(
        "One or more of the variables declared in the recipe are not allowed."
      );
    }
...
export class Deployer {
  ...
  constructor(
    ...
    this.progress = 0;
    this.serverName =
      customMetaData.serverName ||
      globals.txAdmin.globalConfig.serverName ||
      "";
    this.logLines = [];
  ...
  async confirmRecipe(userRecipe) {
    ...
    try {
      ...
      throw new Error(
        `Cannot start() deployer due to a Recipe Error: ${error.message}`
      );
    }
    ...
    try {
      ...
      console.verbose.dir(error);
      throw new Error(
        `Failed to create ${this.deployPath} with error: ${error.message}`
      );
    }
  ...
  async markFailedDeploy() {
    ...
    try {
      ...
      await fse.outputFile(
        filePath,
        "This deploy was failed, please do not use these files."
      );
    } catch (error) {}
  }
  ...
  async runTasks() {
    ...
    const retryTask = async (task, index) => {
      let lastError;
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          const taskTimeoutSeconds =
            task.timeoutSeconds ?? recipeEngine[task.action].timeoutSeconds;
          await Promise.race([
            recipeEngine[task.action].run(
              task,
              this.deployPath,
              contextVariables
            ),
          ...
          if (attempt < 3) {
            ...
            this.customLog(
              `Task failed (retrying ${attempt} of 3): ${error.message}`
            );
    ...
    try {
      ...
      await recipeEngine["replace_string"].run(
        task,
        this.deployPath,
        contextVariables
      );
      ...
      this.customLogError(
        `Failed to replace all vars in server.cfg: ${error.message}`
      );
      return await this.markFailedDeploy();
    }
