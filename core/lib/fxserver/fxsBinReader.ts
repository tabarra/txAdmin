...
//MARK: Extraction
/**
 * Extracts FXServer version info from a binary file buffer.
 ...
 */
const extractFxsVersionInfo = (buffer: Buffer): FxsBinVersionInfo => {
 ...
 * 
 * @param fxsPath - Path to the FXServer installation folder
 * @param isWindows - Whether the current OS is Windows
 * @returns Parsed version info or invalid result
 */
export const readFxsBinVersion = (fxsPath: string, isWindows: boolean): FxsBinVersionInfo => {
    const binaryName = isWindows ? 'FXServer.exe' : 'FXServer';
    const binaryPath = path.join(fxsPath, binaryName);

    try {
        const buffer = fs.readFileSync(binaryPath);
        return extractFxsVersionInfo(buffer);
    } catch {
        return {
            valid: false,
            branch: null,
