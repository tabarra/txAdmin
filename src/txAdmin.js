...
const setGlobal = (profile) => {
    globals[profile] = {
        serverProfile: profile, //FIXME: no need
        adminVault: null,
        discordBot: null,
        fxRunner: null,
        logger: null,
...
module.exports = class txAdmin {
    constructor(serverProfile) {
        ...
        globals[serverProfile].info.serverProfilePath = profilePath;

        //Load Config Vault
        let profileConfig;
        try {
            const ConfigVault = require('./components/configVault');
            globals[serverProfile].configVault = new ConfigVault(profilePath, serverProfile);
            profileConfig = globals[serverProfile].configVault.getAll();
            globals[serverProfile].config = profileConfig.global;
        } catch (err) {
        ...
        if (this.serverProfile === 'sv1.profile') {
            printBanner();
        }
    ...
    //==============================================================
    //NOTE: actually global
    async startAdminVault() {
        if (this.serverProfile !== 'sv1.profile') return;
        const AdminVault = require('./components/adminVault');
        universal.adminVault = new AdminVault();
    }
    ...
    async startDiscordBot(config) {
        ...
        universal.discordBot = new DiscordBot(config);
    }
    ...
    async startFXRunner(config) {
        ...
        globals[this.serverProfile].fxRunner = new FXRunner(config, this.serverProfile);
    }
    ...
    async startLogger(config) {
        ...
        globals[this.serverProfile].logger = new Logger(config, this.serverProfile);
    }
    ...
    async startDynamicAds(config) {
        ...
        universal.dynamicAds = new DynamicAds(config);
    }
    ...
    async startMonitor(config) {
        ...
        globals[this.serverProfile].monitor = new Monitor(config, this.serverProfile);
    }
    ...
    async startStatsCollector(config) {
        ...
        globals[this.serverProfile].statsCollector = new StatsCollector(config, this.serverProfile);
    }
    ...
    async startTranslator() {
        ...
        universal.translator = new Translator();
    }
    ...
    async startWebServer(config) {
        ...
        universal.webServer = new WebServer(config);
    }
    ...
    async startPlayerController(config) {
        ...
        globals[this.serverProfile].playerController = new PlayerController(config, this.serverProfile);
    }
};
