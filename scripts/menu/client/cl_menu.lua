...
local function toggleSpectate(targetPed, targetPlayerId)
  ...
  if isSpectateEnabled then
    ...
    clearPersistentAlert('spectateCtx')
    ...
    sendPersistentAlert('spectateCtx', 'info', ('You are spectating: %s'):format(GetPlayerName(targetPlayerId)))
...
RegisterCommand('txAdmin:menu:endSpectate', function()
  if isSpectateEnabled then
    ...
    preparePlayerForSpec(false)
    sendSnackbarMessage('info', 'Exiting spectate mode')
  end
