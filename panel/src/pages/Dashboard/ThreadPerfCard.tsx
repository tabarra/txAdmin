...
const ThreadPerfChart = memo(({ data, minTickIntervalMarker }: ThreadPerfChartProps) => {
    ...
    return (
        <ResponsiveBar
            ...
            //FIXME: this is covered by the navbar - solution is to ditch the tooltip and add some overlay over the chart
            tooltip={(datum) => {
                const lowerLimit = data.find((_, index) => index === datum.index - 1)?.bucket ?? 0;
                const upperLimit = datum.data.bucket;
                const pctString = (datum.value * 100).toFixed(2) + '%';
                return (
                    <div
                        className="p-3 text-gray-900 bg-white rounded-md shadow-md"
                    >
                        <div>
...
export default function ThreadPerfCard({ data }: ThreadPerfCardProps) {
    return (
        <div className="py-2 rounded-lg border shadow-sm flex flex-col col-span-3">
            ...
            <div className='size-full fill-primary'>
